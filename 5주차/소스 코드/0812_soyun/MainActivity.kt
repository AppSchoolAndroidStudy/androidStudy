...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        testViewModel.run{
            ...
            testData.observe(this@MainActivity){
                Toast.makeText(this@MainActivity, "testIdx: ${testData.value?.testIdx}, testData1: ${testData.value?.testData1}, testData2: ${testData.value?.testData2}", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    inner class TestRecyclerViewAdapter : RecyclerView.Adapter<TestRecyclerViewAdapter.TestViewHolder>(){
        ...
        override fun onBindViewHolder(holder: TestViewHolder, position: Int) {
            holder.textViewRow1.text = testViewModel.testDataList.value?.get(position)?.testIdx.toString()
            holder.textViewRow1.setOnClickListener {
                testViewModel.testDataList.value?.get(position)?.testIdx?.let { it1 -> testViewModel.setSelectedData(this@MainActivity, it1) }
            }
        }
    }
